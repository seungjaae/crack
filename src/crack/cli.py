...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.no_dxf:
        written.append(export.write_dxf(result.segments, cfg, result.raster,
                                       out_dir / f"{prefix}.dxf"))
    if not args.no_tsv:
        written.append(export.write_tsv(result.segments, cfg, out_dir / f"{prefix}.tsv"))
    if not args.no_preview:
        written.append(export.write_preview(result.segments, cfg, result.raster,
                                            out_dir / f"{prefix}_preview.png"))

    if not args.quiet:
        print()
        print(export.summarize(result.segments, cfg))
